...
struct VoiceIntroView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 24) {
                    ...
                    if let existingUrl = auth.user?.voiceIntroUrl, !existingUrl.isEmpty, recordedURL == nil {
                        Button { playExistingIntro(existingUrl) } label: {
                            HStack(spacing: 8) {
                                Image(systemName: isPlayingExisting ? "stop.fill" : "play.fill")
                                Text(isPlayingExisting ? "Stop Playback" : "Listen to current intro").font(.subheadline.bold())
                            }
                    ...
                    Button { togglePlayback() } label: {
                        HStack(spacing: 8) {
                            ...
                            Text(isPlaying ? "Stop Preview" : "Preview Recording").font(.subheadline.bold())
                        }
                        .foregroundColor(Color(hex: "5F7A66")).frame(maxWidth: .infinity).padding(.vertical, 14)
                        .overlay(RoundedRectangle(cornerRadius: 16).stroke(Color(hex: "D6D0C9"), lineWidth: 1))
                    }
                    .disabled(recordedURL == nil).opacity(recordedURL == nil ? 0.5 : 1)

                    Button { uploadVoiceIntro() } label: {
                        HStack(spacing: 8) {
                            if isUploading { ProgressView().tint(.white) }
                            Image(systemName: "arrow.up.circle.fill")
                            Text(isUploading ? "Uploading..." : "Upload & Continue").font(.headline)
                        }
                        .foregroundColor(.white).frame(maxWidth: .infinity).padding(.vertical, 16)
                    ...
                    Text("Tip: Share something unique about yourself — your hobbies, what makes you laugh, or what you're looking for.")
                        .font(.caption).foregroundColor(Color(hex: "6C6C6C")).multilineTextAlignment(.center)
                }
        ...
        .navigationTitle("Voice Intro").navigationBarTitleDisplayMode(.inline)
        .alert("Voice Intro", isPresented: $showAlert) {
            Button("OK") { if alertMessage.contains("success") { dismiss() } }
        } message: { Text(alertMessage) }
        .onDisappear { timer?.invalidate(); audioRecorder?.stop(); audioPlayer?.stop() }
    }
    ...
    private var statusBadge: some View {
        HStack(spacing: 6) {
            if isRecording {
                ...
                Image(systemName: "mic.slash").foregroundColor(Color(hex: "7D8B82"))
                Text("No recording yet")
            }
        }
        .font(.caption.bold()).foregroundColor(Color(hex: "5F7A66"))
        .padding(.horizontal, 12).padding(.vertical, 6)
        .background(Color(hex: "ECE0D7")).clipShape(Capsule())
    }
    ...
    private func startRecording() {
        ...
        do {
            ...
            alertMessage = "Could not access microphone."; showAlert = true; return
        }
        ...
        do {
            ...
            isRecording = true; duration = 0; recordedURL = nil
            timer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
                duration = audioRecorder?.currentTime ?? 0
                if duration >= maxDuration { stopRecording() }
            }
        } catch {
            alertMessage = "Failed to start recording."; showAlert = true
        }
    }
    ...
    private func togglePlayback() {
        ...
        if isPlaying { audioPlayer?.stop(); isPlaying = false }
            ...
            } catch { alertMessage = "Could not play recording."; showAlert = true }
        }
    }
    ...
    private func playExistingIntro(_ urlString: String) {
        ...
        Task {
            do {
                ...
                audioPlayer?.play(); isPlayingExisting = true
                let playDuration = audioPlayer?.duration ?? 15
                DispatchQueue.main.asyncAfter(deadline: .now() + playDuration) { isPlayingExisting = false }
            } catch { alertMessage = "Could not play existing intro."; showAlert = true }
        }
    }
    ...
    private func uploadVoiceIntro() {
        ...
        Task {
            ...
            isUploading = false; showAlert = true
        }
    }
