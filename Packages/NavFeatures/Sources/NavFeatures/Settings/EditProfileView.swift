...
struct EditProfileView: View {
        ...
        ScrollView {
            VStack(spacing: 24) {
                                    ...
                                    Button { lookingFor = option.0 } label: {
                                        HStack(spacing: 14) {
                                            Image(systemName: option.2).font(.title3)
                                                ...
                                                .clipShape(Circle())
                                            Text(option.1).font(.subheadline).foregroundColor(AppColors.textPrimary)
                                            Spacer()
                                            if lookingFor == option.0 {
                                                Image(systemName: "checkmark.circle.fill").foregroundColor(AppColors.editAccent)
                                            }
                                        }
                                        .padding(12)
                ...
                Button { saveProfile() } label: {
                    ...
                    .background(LinearGradient(colors: [AppColors.editAccent, AppColors.secondary], startPoint: .leading, endPoint: .trailing))
            ...
            HStack(spacing: 12) {
                ...
                VStack(spacing: 12) {
                    ForEach(1..<3) { index in
                        RoundedRectangle(cornerRadius: 16).fill(AppColors.inputBackground).frame(height: 94)
                            .overlay {
    ...
    private var addPhotoPlaceholder: some View {
        VStack(spacing: 8) {
            Image(systemName: "camera.fill").font(.title).foregroundColor(AppColors.editAccent)
            Text("Add Photo").font(.caption).foregroundColor(AppColors.textSecondary)
        }
    }

    private var smallAddPlaceholder: some View {
        Image(systemName: "plus").font(.title3).foregroundColor(AppColors.textMuted)
    }
    ...
    private func formSection(title: String, @ViewBuilder content: () -> some View) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            Text(title).font(.title3.bold()).foregroundColor(AppColors.textPrimary)
            content()
        }
    ...
    private func formField(title: String, @ViewBuilder content: () -> some View) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(title).font(.subheadline.bold()).foregroundColor(AppColors.textSecondary)
            content()
        }
    }
    ...
    private func chipToggle(label: String, isSelected: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(label).font(.subheadline)
                ...
                .clipShape(Capsule())
                .overlay(Capsule().stroke(isSelected ? AppColors.editAccent : AppColors.border, lineWidth: 1.5))
        }
    }
