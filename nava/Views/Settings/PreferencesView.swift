...
struct PreferencesView: View {
    ...
    var body: some View {
                    ...
                    preferenceSection(title: "What are you looking for?") {
                        FlowLayout(spacing: 10) {
                            ForEach(intentOptions, id: \.0) { option in
                                chipButton(label: option.1, isSelected: intent == option.0) {
                                    intent = option.0
                                }
                            }
                        }
                        ...
                        FlowLayout(spacing: 10) {
                            ForEach(genderOptions, id: \.0) { option in
                                chipButton(label: option.1, isSelected: genders.contains(option.0)) {
                                    if genders.contains(option.0) {
                                        genders.remove(option.0)
                                    } else {
                                        genders.insert(option.0)
                                    }
                                }
                            }
                        }
                    ...
                    preferenceSection(title: "Verification") {
                        Toggle(isOn: $onlyVerified) {
                            HStack(spacing: 8) {
                                ...
                                Text("Only show verified profiles")
                                    .font(.subheadline)
                            }
                        }
        ...
        .alert("Preferences", isPresented: $showAlert) {
            Button("OK") {}
        } message: {
            Text(alertMessage)
        }
    ...
    private func loadPreferences() async {
        ...
        do {
            ...
            if let prefs = result["myPreferences"] as? [String: Any] {
                ...
                if let km = prefs["maxDistanceKm"] as? Int {
                    distance = String(Int(Double(km) * 0.621))
                }
        ...
        Task {
            do {
                ...
                let variables: [String: Any] = [
                    "input": [
                        "minAge": Int(ageMin) ?? 18,
                        "maxAge": Int(ageMax) ?? 50,
                        "maxDistanceKm": distanceKm,
                        "preferredGenders": Array(genders),
